...
function NotaCell({ valor }) {
  ...
  return (
    <span
      className={`fw-medium small ${aprovado ? "text-success" : "text-danger"}`}
    >
      {fmt(valor)}
    </span>
  );
      ...
      <Card.Header className="bg-white border-bottom py-2 px-4 d-flex justify-content-between align-items-center">
        ...
        <div className="d-flex align-items-center gap-3">
          ...
          <small className="text-muted">
            {frequencia.totalFaltas} falta(s)
          </small>
        </div>
      </Card.Header>
  ...
  return (
    <Card className="border-0 shadow-sm mb-3">
      <Card.Header className="bg-white border-bottom py-2 px-4 d-flex justify-content-between align-items-center">
        ...
        {resultado && (
          <Badge bg={resultado === "Aprovado" ? "success" : "danger"}>
            {resultado}
          </Badge>
        )}
      </Card.Header>
          ...
          <tbody>
            {notas.map((n) => (
              <tr key={n.disciplinaId}>
                ...
                <td>
                  <NotaCell valor={n.unidade1} />
                </td>
                <td>
                  <NotaCell valor={n.unidade2} />
                </td>
                <td>
                  <NotaCell valor={n.unidade3} />
                </td>
                <td>
                  <NotaCell valor={n.media1Semestre} />
                </td>
                <td>
                  <NotaCell valor={n.unidade4} />
                </td>
                <td>
                  <NotaCell valor={n.unidade5} />
                </td>
                <td>
                  <NotaCell valor={n.unidade6} />
                </td>
                <td>
                  <NotaCell valor={n.media2Semestre} />
                </td>
                <td>
                  <NotaCell valor={n.mediaAnual} />
                </td>
                <td>
                  <NotaCell valor={n.notaRecuperacao} />
                </td>
                <td>
                  <NotaCell valor={n.mediaFinal} />
                </td>
                <td>
                  {n.resultado ? (
                    <Badge
                      bg={n.resultado === "Aprovado" ? "success" : "danger"}
                      className="small"
                    >
                      {n.resultado}
                    </Badge>
  ...
  return (
    <Card className="border-0 shadow-sm mb-3">
      <Card.Header className="bg-white border-bottom py-2 px-4">
        <small className="fw-semibold text-slate-700">
          Desenvolvimento por Bimestre
        </small>
      </Card.Header>
      <Card.Body className="p-3">
        <Row className="g-3">
...
export default function Diario() {
    ...
    Promise.all([getTurmas(), getAnoLetivos()])
      .then(([t, a]) => {
        ...
        if (a.data.data.length > 0)
          setAnoLetivoSelecionado(String(a.data.data[0].id));
      })
  ...
  useEffect(() => {
    ...
    const loadFn =
      turma.segmento === SEGMENTOS.Maternal
        ? getDiarioMaternal
